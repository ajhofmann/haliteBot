...
if(gameMap.getSite(goalLoc).strength < 75):
    while True:
        ...
        for loc in myLocations:
            totalStrength += gameMap.getSite(loc).strength
        ...
        if(gameMap.getSite(myHead).strength > strengthNeeded):
            ...
            myLocations.append(myHead)
        if(totalStrength > strengthNeeded - 5*(len(myLocations)-1)):
            # move strength towards the head
            for i in range(len(myLocations)-1):
